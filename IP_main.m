%% ----------------------------------------------------------------------------------
%% Implementing shape-adaptive tensor factorization (SATF) for Indian Pines data sets
%% ----------------------------------------------------------------------------------
close all
clear all
clc
    ...
    for i_classifier_type=1:length(classifier_type_all)
        ...
        for i_DR = 1:length(DR_method_all)
            ...
            for i_MC = 1:MMiter
                ...
                elseif strcmp(DR_method,'Origin')% no DR
                ...
                for ii_DR = i_DR_all
                    ...
                    output_folder = sprintf('%s%s%s%s%d%s%s%s%s',pwd,'\results_IP0501\',neighorhood_method,'_',patchSize,'_',DR_method,'_',classifier_type);
